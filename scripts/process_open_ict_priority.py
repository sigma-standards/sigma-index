#!/usr/bin/env python3
"""Build processed OASIS, Ecma, and GS1 priority records."""
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    if not rows:
        raise ValueError("open ICT priority source has no rows")
    ...
    if duplicate_ids:
        raise ValueError(f"duplicate open ICT priority sigma_id values: {', '.join(sorted(duplicate_ids))}")
...
def process_open_ict_priority(
    repo_root: Path,
    source_path: Path | None = None,
    output_path: Path | None = None,
) -> Path:
    source = source_path or repo_root / "data" / "reference" / "open_ict_priority_sources.csv"
    output = output_path or repo_root / "data" / "processed" / "open_ict_standards.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build OASIS, Ecma, and GS1 priority records.")
    ...
    try:
        output_path = process_open_ict_priority(args.repo_root, args.source_path, args.output_path)
    except Exception as exc:
        print(f"Open ICT priority ingestion failed: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote open ICT priority records to {output_path}")
    return 0
