...
PROGRESS_REPORT = Path("data/reports/project_progress.csv")
PROGRESS_DOC_OUTPUT = Path("docs/PROJECT_PROGRESS.md")
...
def build_progress(root: Path | str = Path(".")) -> dict[str, object]:
    root = Path(root)
    ...
    full_vision_percent = int(round(100 * done / total_tasks)) if total_tasks else 0
    public_mvp_percent = int(round(100 * (done + active) / total_tasks)) if total_tasks else 0
    return {
...
def write_progress_report(path: Path, stats: dict[str, object], tasks: list[dict[str, str]]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        writer.writerow({field: stats[field] for field in fieldnames})

    write_progress_doc(path.parents[1].parent / PROGRESS_DOC_OUTPUT, stats, tasks)


def write_progress_doc(path: Path, stats: dict[str, object], tasks: list[dict[str, str]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        ...
        "",
        f"- Total roadmap tasks: {stats['total_tasks']}",
        f"- Completed tasks: {stats['done']}",
        f"- Active tasks: {stats['active']}",
        f"- Planned tasks: {stats['planned']}",
        f"- Blocked tasks: {stats['blocked']}",
        "",
        f"- Full vision progress: {stats['full_vision_percent']}% complete",
        f"- Public MVP readiness: {stats['public_mvp_percent']}% done or active",
        "",
        "## Progress by phase",
        "",
    ...
    for phase in sorted(phases, key=lambda value: int(value) if value.isdigit() else 999):
        phase_tasks = phases[phase]
        phase_counts = Counter(task["status"] for task in phase_tasks)
        status = "active" if phase_counts.get("active") else "done" if phase_counts.get("done") else "planned"
        lines.append(
            f"| {phase} | {status} | {phase_counts.get('done', 0)} | {phase_counts.get('active', 0)} | {phase_counts.get('planned', 0)} | {phase_counts.get('blocked', 0)} |"
        )
    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- This report is regenerated by `scripts/build_research_task_report.py`.")
    lines.append("")
...
def build_report(root: Path | str = Path(".")) -> Path:
    ...
    progress_stats = build_progress(root)
    write_progress_report(root / PROGRESS_REPORT, progress_stats, tasks)
...
def write_markdown(path: Path, tasks: list[dict[str, str]]) -> None:
    ...
    planned = statuses.get("planned", 0)
    blocked = statuses.get("blocked", 0)
    full_percent = int(round(100 * done / total_tasks)) if total_tasks else 0
    public_percent = int(round(100 * (done + active) / total_tasks)) if total_tasks else 0
    ...
    lines = [
        ...
        "## Project Progress",
        "",
        f"- Total roadmap tasks: {total_tasks}",
        f"- Completed tasks: {done}",
        f"- Active tasks: {active}",
        f"- Planned tasks: {planned}",
        f"- Blocked tasks: {blocked}",
        "",
        f"- Full vision progress: {full_percent}% complete",
        f"- Public MVP readiness: {public_percent}% done or active",
        "",
    ]
