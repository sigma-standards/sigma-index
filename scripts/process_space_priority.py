#!/usr/bin/env python3
"""Build processed CCSDS and ECSS space priority records."""
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    if not rows:
        raise ValueError("space priority source has no rows")
    ...
    if duplicate_ids:
        raise ValueError(f"duplicate space priority sigma_id values: {', '.join(sorted(duplicate_ids))}")
...
def process_space_priority(
    repo_root: Path,
    source_path: Path | None = None,
    output_path: Path | None = None,
) -> Path:
    source = source_path or repo_root / "data" / "reference" / "space_priority_sources.csv"
    output = output_path or repo_root / "data" / "processed" / "space_standards.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build CCSDS and ECSS space priority records.")
    ...
    try:
        output_path = process_space_priority(args.repo_root, args.source_path, args.output_path)
    except Exception as exc:
        print(f"Space priority ingestion failed: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote space priority records to {output_path}")
    return 0
