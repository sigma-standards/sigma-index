#!/usr/bin/env python3
"""Build processed NIST cybersecurity and AI priority records."""
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    if not rows:
        raise ValueError("NIST priority source has no rows")
    ...
    if duplicate_ids:
        raise ValueError(f"duplicate NIST priority sigma_id values: {', '.join(sorted(duplicate_ids))}")
...
def process_nist_priority(
    repo_root: Path,
    source_path: Path | None = None,
    output_path: Path | None = None,
) -> Path:
    source = source_path or repo_root / "data" / "reference" / "nist_priority_sources.csv"
    output = output_path or repo_root / "data" / "processed" / "nist_priority_standards.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build NIST cybersecurity and AI priority records.")
    ...
    try:
        output_path = process_nist_priority(args.repo_root, args.source_path, args.output_path)
    except Exception as exc:
        print(f"NIST priority ingestion failed: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote NIST priority records to {output_path}")
    return 0
