#!/usr/bin/env python3
"""Build processed Phase 2A health priority records from curated sources."""
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    if not rows:
        raise ValueError("health priority source has no rows")
    ...
    if duplicate_ids:
        raise ValueError(f"duplicate health priority sigma_id values: {', '.join(sorted(duplicate_ids))}")
...
def process_health_priority(
    repo_root: Path,
    source_path: Path | None = None,
    output_path: Path | None = None,
) -> Path:
    source = source_path or repo_root / "data" / "reference" / "health_priority_sources.csv"
    output = output_path or repo_root / "data" / "processed" / "health_priority_standards.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Phase 2A health priority processed records.")
    ...
    try:
        output_path = process_health_priority(args.repo_root, args.source_path, args.output_path)
    except Exception as exc:
        print(f"health priority ingestion failed: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote health priority records to {output_path}")
    return 0
