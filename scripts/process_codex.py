#!/usr/bin/env python3
"""Build processed Phase 2B Codex priority records from curated sources."""
...
def validate_rows(rows: list[dict[str, str]]) -> None:
    if not rows:
        raise ValueError("Codex priority source has no rows")
    ...
    if duplicate_ids:
        raise ValueError(f"duplicate Codex priority sigma_id values: {', '.join(sorted(duplicate_ids))}")
...
def process_codex(
    ...
    source = source_path or repo_root / "data" / "reference" / "codex_priority_sources.csv"
    output = output_path or repo_root / "data" / "processed" / "codex_standards.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Phase 2B Codex priority processed records.")
    ...
    try:
        output_path = process_codex(args.repo_root, args.source_path, args.output_path)
    except Exception as exc:
        print(f"Codex priority ingestion failed: {exc}", file=sys.stderr)
        return 1

    print(f"Wrote Codex priority records to {output_path}")
    return 0
